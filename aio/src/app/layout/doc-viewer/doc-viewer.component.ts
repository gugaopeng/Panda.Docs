...
export class DocViewerComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.docContents$.pipe(switchMap(newDoc => this.render(newDoc)), takeUntil(this.onDestroy$), )
        .subscribe();
  }
  ...
  protected prepareTitleAndToc(targetElem: HTMLElement, docId: string): () => void {
    ...
    return () => {
      ...
      // TODO: 设置Title的位置
      this.titleService.setTitle(title ? `经纪星球 - ${title}` : '经纪星球');
    };
  }
  ...
  protected render(doc: DocumentContents): Observable<void> {
    ...
    return this.void$.pipe(
        ...
        tap(() => this.docReady.emit()), switchMap(() => this.swapViews(addTitleAndToc)),
        tap(() => this.docRendered.emit()), catchError(err => {
  ...
  protected swapViews(onInsertedCb = () => {}): Observable<void> {
    ...
    const animateProp =
        (elem: HTMLElement, prop: keyof CSSStyleDeclaration, from: string, to: string,
         duration = 200) => {
          ...
          return animationsDisabled ?
              ...
              this.void$.pipe(
                  ...
                  // setting each style.
                  switchMap(() => raf$), tap(() => elem.style[prop] = from), switchMap(() => raf$),
    ...
    return done$.pipe(
        ...
        tap(() => this.hostElement.appendChild(this.nextViewContainer)), tap(() => onInsertedCb()),
        tap(() => this.docInserted.emit()), switchMap(() => animateEnter(this.nextViewContainer)),
